...
function RecipeInProgress() {
  ...
  const handleIngredientCheck = (ingredient, measure) => {
    const isChecked = checkedIngredients.includes(ingredient);
    if (isChecked) {
      setCheckedIngredients(checkedIngredients.filter((item, measures) => item
      !== ingredient && measures !== measure));
    } else {
      ...
      <ul>
        {Object.keys(recipe).map((key) => {
          if (key.includes('Ingredient') && recipe[key]) {
            ...
            return (
              <li key={ ingredientIndex } style={ ingredientStyle }>
                <label data-testid={ `${ingredientIndex}-ingredient-step` }>
                  <input
                    type="checkbox"
                    checked={ isChecked }
                    onChange={ () => handleIngredientCheck(ingredient, measure) }
                  />
                  {`${ingredient} - ${measure}`}
                </label>
              </li>
